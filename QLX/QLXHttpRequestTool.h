...
// 这个data类型  一般是字典 
typedef void (^HttpResponse)(id data , NSError * error) ;   // 请求结果闭包
typedef void (^HttpProgress)(double progress);              // 请求过程闭包

@interface QLXHttpRequestTool : NSObject

/**
 *  GET类型 普通接口请求
 *  HttpResponse data 一般是字段类型
 *  @param url      请求链接
 *  @param params   请求参数
...
/**
 *  下载文件
 *  注意：HttpResponse 中 data 是文件保存路径 nsstring类型
 *  @param url        接口
